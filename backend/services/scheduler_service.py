...
def schedule_single_video(video_id):
    ...
    try:
        ...
        # Instagram channels get the logo'd variant; everyone else (YouTube,
        # TikTok) gets the plain one. Falls back to the standard file if no
        # Instagram variant was uploaded for this clip (e.g. only one file
        # got dropped into incoming/).
        if channel.platform == "instagram" and video.r2_url_instagram:
            video_url = video.r2_url_instagram
        else:
